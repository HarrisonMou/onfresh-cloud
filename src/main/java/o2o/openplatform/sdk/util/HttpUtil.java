...
public class HttpUtil {
	...
	public static String sendSimplePostRequest(String url, Map<String, Object> param) throws Exception {
		...
		try {
			...
			LOGGER.error("调用HttpsUtil.sendSimplePostRequest ConnectTimeoutException, url=" + url, e.getMessage());
			e.printStackTrace();
			throw e;
		} catch (SocketTimeoutException e) {
			result = "{\"code\":\"-1\", \"msg\":\"请求响应超时\"}";
			LOGGER.error("调用HttpUtil.sendSimplePostRequest SocketTimeoutException, url=" + url, e.getMessage());
			e.printStackTrace();
			throw e;
		} catch (IOException e) {
			result = "{\"code\":\"-1\", \"msg\":\"请求响应超时\"}";
			LOGGER.error("调用HttpUtil.sendSimplePostRequest IOException, url=" + url, e.getMessage());
			e.printStackTrace();
			throw e;
		} catch (Exception e) {
			result = "{\"code\":\"-1\", \"msg\":\"服务开小差，系统努力修复中\"}";
			LOGGER.error("调用HttpUtil.sendSimplePostRequest Exception, url=" + url, e.getMessage());
			e.printStackTrace();
			throw e;
		} finally {
	...
	public static String sendSimpleGetRequest(String url, Map<String, Object> param) throws Exception {
		...
		try {
			...
			result = "{\"code\":\"-1\", \"msg\":\"请求响应超时\"}";
			LOGGER.error("HttpUtil.sendSimpleGetRequest响应超时, url=" + url, ex);
			throw ex;
		} catch (Exception ex) {
			result = "{\"code\":\"-1\", \"msg\":\"服务开小差，系统努力修复中\"}";
			LOGGER.error("调用HttpUtil.sendSimpleGetRequest Exception, url=" + url, ex);
			throw ex;
		} finally {
			if (get != null) {
